...
#include <FirebaseESP32.h>    // Librería para conectar a Firebase
#include <vector>
#include <utility>
...
const char *ruta = "/Ejemplo_03"; // Cambiado a const char* para ahorrar espacio
...
void sendToServer(double voltaje, double corriente, double potencia, double energia)
{
  if (WiFi.status() == WL_CONNECTED)
  {
    ...
    {                 // 443 is the port for HTTPS
      ...
      String postData = "POST /save_data_p3.php HTTP/1.1\r\n";
...
std::vector<std::pair<double, double>> voltageRanges = {
    ...
    {98.87, 1.0097}, {108.7, 1.0106}, {112.601, 1.0098},{117.273, 1.0122},
    {125.7, 1.0113}, {127, 1},
...
std::vector<std::pair<double, double>> currentRanges = {
    {0.061, 0.01}, {0.123, 0.03}, {0.245, 0.6632}, {0.5343, 0.9746}, {0.830816, 0.9621},
    ...
    {5.582476, 0.9655}, {6.135332, 0.9669}, {6.4168, 0.9681}, {6.695516, 0.9683},
    {7.08, 0.945}
};
